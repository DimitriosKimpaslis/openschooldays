...
const Header = () => {
    ...
    return (
        <div className=' text-gray-50 w-full flex justify-between items-center py-6 px-10 absolute z-50'>
            ...
            {user ? <p className='cursor-pointer' onClick={() => navigate('/profile')}> {user.email}</p> : <button onClick={() => navigate('/signin')} className='bg-newSomon px-12 py-2 text-xl w-fit h-fit hover:bg-newPurple text-newPurple hover:text-newSomon transition-colors duration-400'>REGISTER NOW <TrendingFlatIcon className='relative bottom-[2px]' /></button>}
        </div>
    )
}

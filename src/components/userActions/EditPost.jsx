...
const EditPost = () => {
    ...
    const getPost = async () => {
        ...
        setTitle({ value: post.title, rows: 1 })
    ...
    useEffect(() => {
        if (title.value.length > 0) {
            localStorage.setItem('edit-title', JSON.stringify(title));
        }
    }, [title])

    useEffect(() => {
        if (thumbnail !== 'https://placehold.co/600x400/png') {
            localStorage.setItem('edit-thumbnail', thumbnail);
        }
    }, [thumbnail])
